var $jWY6v$path = require("path");
var $jWY6v$parcelplugin = require("@parcel/plugin");
var $jWY6v$parcelsourcemap = require("@parcel/source-map");
var $jWY6v$less = require("less");
...
$parcel$export(module.exports, "default", () => $0ef8d399d7f4ab38$export$2e2bcd8739ae039);





async function $ac096a7a3ff2cd9c$export$11e63f7b0f3d9900({ config: config  }) {
    ...
    if (configFile != null) {
        ...
        if (configContents.paths) configContents.paths = configContents.paths.map((p)=>(0, ($parcel$interopDefault($jWY6v$path))).resolve((0, ($parcel$interopDefault($jWY6v$path))).dirname(configFile.filePath), p));
    }
    ...
    let isDynamic = configFile && (0, ($parcel$interopDefault($jWY6v$path))).extname(configFile.filePath) === ".js";
...
const $0ef8d399d7f4ab38$var$WEBPACK_ALIAS_RE = /^~[^/]/;
var $0ef8d399d7f4ab38$export$2e2bcd8739ae039 = new (0, $jWY6v$parcelplugin.Transformer)({
    loadConfig ({ config: config  }) {
        return (0, $ac096a7a3ff2cd9c$export$11e63f7b0f3d9900)({
            config: config
        });
    },
    async transform ({ asset: asset , options: options , config: config , resolve: resolve  }) {
        ...
        try {
            ...
            lessConfig.plugins = [
                ...lessConfig.plugins || [],
                $0ef8d399d7f4ab38$var$urlPlugin({
                    asset: asset
                }),
                $0ef8d399d7f4ab38$var$resolvePathPlugin({
                    asset: asset,
                    resolve: resolve
                })
            ];
            result = await (0, ($parcel$interopDefault($jWY6v$less))).render(code, lessConfig);
        } catch (err) {
        ...
        if (result.map != null) {
            let map = new (0, ($parcel$interopDefault($jWY6v$parcelsourcemap)))(options.projectRoot);
            let rawMap = JSON.parse(result.map);
            map.addVLQMap({
                ...rawMap,
                sources: rawMap.sources.map((s)=>(0, ($parcel$interopDefault($jWY6v$path))).relative(options.projectRoot, s))
            });
            asset.setMap(map);
        }
...
function $0ef8d399d7f4ab38$var$urlPlugin({ asset: asset  }) {
    return {
...
function $0ef8d399d7f4ab38$var$resolvePathPlugin({ asset: asset , resolve: resolve  }) {
    return {
        install (less, pluginManager) {
            class LessFileManager extends less.FileManager {
                ...
                async loadFile(rawFilename, currentDirectory, options) {
                    ...
                    if ($0ef8d399d7f4ab38$var$WEBPACK_ALIAS_RE.test(filename)) {
                    ...
                    if (filename[0] !== "~") outer: for (let p of paths)for (let prefix of prefixes){
                        ...
                        if (p) filePath = (0, ($parcel$interopDefault($jWY6v$path))).join(p, filePath);
