var $4bxjF$path = require("path");
var $4bxjF$parcelplugin = require("@parcel/plugin");
var $4bxjF$parcelsourcemap = require("@parcel/source-map");
var $4bxjF$less = require("less");
...
$parcel$export(module.exports, "default", () => $71ba1bd35ebce10c$export$2e2bcd8739ae039);





async function $858f3d8835fe53fd$export$11e63f7b0f3d9900({ config: config  }) {
    ...
    if (configFile != null) {
        ...
        if (configContents.paths) configContents.paths = configContents.paths.map((p)=>(0, ($parcel$interopDefault($4bxjF$path))).resolve((0, ($parcel$interopDefault($4bxjF$path))).dirname(configFile.filePath), p));
    }
    ...
    let isDynamic = configFile && (0, ($parcel$interopDefault($4bxjF$path))).extname(configFile.filePath) === ".js";
...
const $71ba1bd35ebce10c$var$WEBPACK_ALIAS_RE = /^~[^/]/;
var $71ba1bd35ebce10c$export$2e2bcd8739ae039 = new (0, $4bxjF$parcelplugin.Transformer)({
    loadConfig ({ config: config  }) {
        return (0, $858f3d8835fe53fd$export$11e63f7b0f3d9900)({
            config: config
        });
    },
    async transform ({ asset: asset , options: options , config: config , resolve: resolve  }) {
        ...
        try {
            ...
            lessConfig.plugins = [
                ...lessConfig.plugins || [],
                $71ba1bd35ebce10c$var$urlPlugin({
                    asset: asset
                }),
                $71ba1bd35ebce10c$var$resolvePathPlugin({
                    asset: asset,
                    resolve: resolve
                })
            ];
            result = await (0, ($parcel$interopDefault($4bxjF$less))).render(code, lessConfig);
        } catch (err) {
        ...
        if (result.map != null) {
            let map = new (0, ($parcel$interopDefault($4bxjF$parcelsourcemap)))(options.projectRoot);
            let rawMap = JSON.parse(result.map);
            map.addVLQMap({
                ...rawMap,
                sources: rawMap.sources.map((s)=>(0, ($parcel$interopDefault($4bxjF$path))).relative(options.projectRoot, s))
            });
            asset.setMap(map);
        }
...
function $71ba1bd35ebce10c$var$urlPlugin({ asset: asset  }) {
    return {
...
function $71ba1bd35ebce10c$var$resolvePathPlugin({ asset: asset , resolve: resolve  }) {
    return {
        install (less, pluginManager) {
            class LessFileManager extends less.FileManager {
                ...
                async loadFile(rawFilename, currentDirectory, options) {
                    ...
                    if ($71ba1bd35ebce10c$var$WEBPACK_ALIAS_RE.test(filename)) {
                    ...
                    if (filename[0] !== "~") outer: for (let p of paths)for (let prefix of prefixes){
                        ...
                        if (p) filePath = (0, ($parcel$interopDefault($4bxjF$path))).join(p, filePath);
